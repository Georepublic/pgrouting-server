...
CREATE OR REPLACE VIEW public.road_profile_1 AS 
 SELECT osm.id AS gid, cls.title AS class, round(osm.km * 1000::double precision) AS length, osm.osm_name AS name, 
   ...
   LEFT JOIN ( SELECT a.speed, b.defaultspeed, a.enabled AS cfg_enabled, b.enabled AS cls_enabled, a.pid, a.cid AS class, b.tag, b.title
...
ALTER TABLE public.road_profile_1 OWNER TO postgres;
